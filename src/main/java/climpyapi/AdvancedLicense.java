...
public class AdvancedLicense {
	...
	public boolean register(){
		//log(0, "[Climpy] (API) Kontrol ediliyor..");
		ValidationType vt = isValid();
		if(vt == ValidationType.VALID){
			//log(1, "[Climpy] (API)  Kontrol basarili, eklentiyi senin icin aciyorum.");
			return true;
		}else{
			//log(1, "[Climpy] (API) Kontrol gecerisiz calisiyor.");
			//log(1, "[Climpy] (API) Durum: "+vt.toString());
			log(1, "[Climpy] (API) Eklentiyi kapatiyorum.");


			getInstance().getScheduler().cancel(plugin);
			return false;
		}
	}
	...
	public ValidationType isValid(){
		...
		try{
			URL url = new URL(validationServer+"?v1="+xor(rand, sKey)+"&v2="+xor(rand, key)+"&pl="+plugin.getDescription());
			if(debug) System.out.println("Aranadi");
